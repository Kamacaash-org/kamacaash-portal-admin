...
const CompletedRejectedOrdersPage = () => {
    ...
    useEffect(() => {
        const initialCompleted = Array.isArray(completedOrdersData) ? completedOrdersData : [];
        const initialCancelled = Array.isArray(cancelledOrdersData) ? cancelledOrdersData : [];
        ...
        const totalRevenue = initialCompleted.reduce((sum, order) => sum + (order.amount || 0), 0);
    ...
    const completedColumns = [
        ...
        },
        {
            ...
            cell: row => (
                <strong className="text-success">#{row.orderId}</strong>
            )
        },
        {
            name: 'CUSTOMER',
    ...
    const cancelledColumns = [
        ...
        },
        {
            ...
            selector: row => row.orderId,
            cell: row => (
                <strong className="text-danger">#{row.orderId}</strong>
            )
        },
        {
            name: 'CUSTOMER',
        ...
        },
        {
            name: 'REASON',
            cell: row => (
                <div className="text-truncate" style={{ maxWidth: '200px' }} title={row.cancellationReason}>
                            ...
                            <CardBody>
                                <div className="d-flex align-items-center">
                                    <div className="flex-grow-1">
                                        <p className="text-uppercase fw-medium text-muted mb-0">Completed Orders</p>
                    ...
                    <Col xl={3} md={6}>
                        <Card className="card-animate">
                            <CardBody>
                                <div className="d-flex align-items-center">
                                    <div className="flex-grow-1">
                                        <p className="text-uppercase fw-medium text-muted mb-0">Cancelled Orders</p>
                    ...
                    <Col xl={3} md={6}>
                        <Card className="card-animate">
                            <CardBody>
                                <div className="d-flex align-items-center">
                                    <div className="flex-grow-1">
                                        <p className="text-uppercase fw-medium text-muted mb-0">Avg Order Value</p>
                        ...
                        <Nav className="nav-pills nav-justified bg-light" role="tablist">
                            <NavItem>
                                ...
                                >
                                    ...
                                    Completed Orders
                                    <Badge color="success" className="ms-2">{filteredCompletedOrders.length}</Badge>
                                </NavLink>
                            </NavItem>
                            <NavItem>
                                <NavLink
                                ...
                                >
                                    ...
                                    Cancelled Orders
                                    <Badge color="danger" className="ms-2">{filteredCancelledOrders.length}</Badge>
                                </NavLink>
                            </NavItem>
                        </Nav>
                            ...
                            <Row className="g-3 align-items-end">
                                    ...
                                    <FormGroup className="mb-0">
                                        ...
                                        <Select
                                            ...
                                            value={{
                                                ...
                                                label: filters.dateRange === 'all' ? 'All Time' :
                                                    filters.dateRange === 'today' ? 'Today' :
                                                        filters.dateRange === 'week' ? 'Last 7 Days' : 'Last 30 Days'
                                            }}
                                            onChange={(opt) => setFilters(prev => ({ ...prev, dateRange: opt.value }))}
                                ...
                                <Col md={2}>
                                    <div className="d-grid mb-3">
                                        <Button
                                            color="light"
                                            onClick={() => setFilters({ search: '', dateRange: 'all', amountRange: 'all' })}
                                        >
                                    ...
                                    <DataTable
                                        ...
                                        noDataComponent={
                                            <div className="text-center py-5">
                                                ...
                                                <h5 className="mt-3">No completed orders found</h5>
                                                <p className="text-muted">
                                                    {filters.search || filters.dateRange !== 'all' || filters.amountRange !== 'all'
                                ...
                                {loading ? (
                                    ...
                                    <DataTable
                                        ...
                                        noDataComponent={
                                            <div className="text-center py-5">
                                                <i className="ri-inbox-line display-4 text-muted"></i>
                                                <h5 className="mt-3">No cancelled orders found</h5>
                                                <p className="text-muted">
                                                    {filters.search || filters.dateRange !== 'all' || filters.amountRange !== 'all'
                                                        ? "Try adjusting your search criteria."
                        ...
                        <Row>
                            <Col md={6}>
                                ...
                                <div className="border rounded p-3">
                                    ...
                                    <p><strong>Amount:</strong> ${selectedOrder.amount || 0}</p>
                                    <p><strong>Status:</strong>
                                        <Badge color={getStatusBadge(selectedOrder.status, selectedOrder.type).color} className="ms-2">
                                            <i className={`${getStatusBadge(selectedOrder.status, selectedOrder.type).icon} me-1`}></i>
                            ...
                            <Col md={6}>
                                ...
                                <div className="border rounded p-3 mb-3">
                                    ...
                                    <p><strong>Pickup Start:</strong> {selectedOrder.package?.pickupStart ? formatDateTime(selectedOrder.package.pickupStart) : 'N/A'}</p>
                                    <p><strong>Pickup End:</strong> {selectedOrder.package?.pickupEnd ? formatDateTime(selectedOrder.package.pickupEnd) : 'N/A'}</p>
                                </div>

                                <h6>
                                    {selectedOrder.type === 'completed' ? 'Completion Details' : 'Cancellation Details'}
                                </h6>
                                <div className="border rounded p-3">
                                    {selectedOrder.type === 'completed' ? (
                                        <>
                                            <p><strong>Completed At:</strong> {formatDateTime(selectedOrder.completedAt)}</p>
                                            <p><strong>Completed By:</strong> {selectedOrder.completedBy || 'System'}</p>
                                            <p><strong>Time Since Completion:</strong> {formatRelativeTime(selectedOrder.completedAt)}</p>
                                        </>
                                    ) : (
                                        <>
                                            <p><strong>Cancelled At:</strong> {formatDateTime(selectedOrder.cancelledAt)}</p>
                                            <p><strong>Cancellation Reason:</strong> {selectedOrder.cancellationReason || 'No reason provided'}</p>
                                            <p><strong>Time Since Cancellation:</strong> {formatRelativeTime(selectedOrder.cancelledAt)}</p>
                                            <Alert color="info" className="mt-2 mb-0">
                                                <i className="ri-information-line me-2"></i>
                                                This order was refunded and package quantity was restored.
                                            </Alert>
                                        </>
                                    )}
                                </div>
